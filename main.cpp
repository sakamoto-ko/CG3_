...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	*transformationMatrixDataSprite = MakeIdenttity4x4();
	...
	for (uint32_t index = 0; index < kNumInstance; ++index) {
		...
		transforms[index].translate = { index * 0.1f,index * 0.1f,10.0f };
	}
	...
	*transformationMatrixData = MakeIdenttity4x4();
	...
	while (msg.message != WM_QUIT) {
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
			...
			for (uint32_t index = 0; index < kNumInstance; ++index) {
				Matrix4x4 worldMatrix = MakeAffineMatrix(transforms[index].scale, Vector3{ 0.0f,0.0f,0.0f }, transforms[index].translate);
				Matrix4x4 worldViewProjectionMatrix = Multiply(worldMatrix, Multiply(cameraMatrix, viewProjectionMatrix));
